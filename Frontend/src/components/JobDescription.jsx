      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <div className='flex items-center gap-2 mt-4'>
            <Badge className="text-blue-700 font-bold " variant={"ghost"}>{singleJob?.jobPosition}</Badge>
            <Badge className="text-[#F83002] font-bold " variant={"ghost"}>{singleJob?.jobType}</Badge>
            <Badge className="text-[#7209b7] font-bold " variant={"ghost"}>{singleJob?.salary}</Badge>
          </div>
        </div>
        <Button onClick={isApplied ? null : applyJobHandler} disabled={isApplied} className={`rounded-lg ${isApplied ? 'bg-gray-600 cursor-not-allowed ' : 'bg-[#7209b7] hover:bg-[#540887] cursor-pointer'}`}>{isApplied ? 'Already Applied' : 'Apply now'}</Button>
      </div>
